...
public class stickyContext : MonoBehaviour
{
    ...
    void Awake()
    {
        memoryLibrary = Memory_Library.Instance;
        if (Instance == null)
        {
            Instance = this;
    ...
    public void ResetContext()
    {
        currentContext = null;
        currentScore = 0f;
        Debug.Log("stickyContext: Context has been reset.");
    }

    public void SetContext(string  id)
    {
        Memory_entry newContext = memoryLibrary.Memories.Find(mem => mem.Id == id);
        currentContext = newContext;
        Debug.Log("stickyContext: Context set to memory ID: " + id);
